...
@pytest.fixture
def mock_fixtures():
    return {"matches": [{"utcDate": "2024-12-15T17:30:00Z", "homeTeam": {"name": "Chelsea"}, "awayTeam": {"name": "Manchester United"}}]}

@patch("app.api_service.get_teams_by_league")
def test_get_teams_by_league(mock_get_teams_by_league, valid_league_code, mock_teams):
...
def test_get_team_info(mock_get_team_info, valid_team_name, valid_league_code):
    mock_get_team_info.return_value = {"name": "Manchester United"}
...
def test_send_email(mock_send_email, valid_email):
    ...
    email_sent = send_email(valid_email, "Test Email", "<h1>Test Content</h1>")
